...
| `-rp <file>` | `-Replace` | この原稿限りの一時的な読み・言い回し調整を行う置換ファイルを指定する。詳細は下記「読み間違いの直し方」を参照 |
...
- **固有名詞や専門用語など、恒常的に間違える語句**: VOICEVOXアプリの「設定」→「読み方＆アクセント辞書」で登録する。読みだけでなくアクセント型も指定でき、一度登録すればエンジン側に永続化されるため、本ツール側での対応は不要（次回以降のすべての原稿に自動で適用される）。
