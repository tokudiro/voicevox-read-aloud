...
# その原稿限りの一時的な読み・言い回し調整（置換ファイルは「検索語=置換後」を1行1組）
...
| — | `--replace <file>`（エイリアス`--rp`） | この原稿限りの一時的な読み・言い回し調整を行う置換ファイルを指定する。詳細は下記「読み間違いの直し方」を参照 |
...
漢字や略語の読み間違いには、用途に応じて2つの直し方がある。

- **固有名詞や専門用語など、恒常的に間違える語句**: VOICEVOXアプリの「設定」→「読み方＆アクセント辞書」で登録する。読みだけでなくアクセント型も指定でき、一度登録すればエンジン側に永続化されるため、`read-aloud`側での対応は不要（次回以降のすべての原稿に自動で適用される）。
- **その原稿限りの、一時的な言い回し調整**: `--replace <file>`オプションを使う。VOICEVOXの辞書に登録するほどでもない一回きりの違和感修正や、原稿ごとに読ませ方を変えたい文脈依存の語句に向く。

置換ファイルは1行に1組、`検索語=置換後`の形式で書く。`#`で始まる行はコメント、空行は無視される。
...
Markdown除去後・チャンク分割前の平文全体に対して、ファイルの記載順に単純文字列置換（正規表現は使わない）を適用する。単語境界の判定はしないため、意図しない部分文字列まで置換されないよう検索語の書き方に注意すること。
